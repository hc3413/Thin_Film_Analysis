...
class XRD_RSM:
    ...
    def plot_2D(self, threshold_filt: float = 0, log_scale: bool = True, x_lim = None, y_lim = None, plot_type: str = 'scatter', ax_unit: str = 'reciprocal'):
        '''
        ...
        for count, q in enumerate(self.qxqz_df):
            ...
            if ax_unit == 'reciprocal':
                ...
                # Set plot values
                x = self.lat_param_df[count]['a']
                y = self.lat_param_df[count]['c']
                z = self.lat_param_df[count]['Intensity']
            ...
            if plot_type == 'scatter':   
                # Set up the plot
                sc = ax.scatter(x_filt, y_filt, c=plot_intensity, cmap='plasma', s=2.5, edgecolors='none')
                ...
                contour = ax.scatter(x_filt, y_filt, c=plot_intensity, cmap='plasma', s=2.5, edgecolors='none')
                ...
                mesh = ax.pcolormesh(grid_x, grid_y, grid_intensity, cmap='plasma', shading='auto')
    ...
    def plot_3D(self, threshold_filt: float = 0, log_scale: bool = True, x_lim = None, y_lim = None, plot_type: str = 'surface'):
        ...
        for count, q in enumerate(self.qxqz_df):
            ...
            if plot_type == 'surface':
                # Create a meshgrid for qx and qz
               
            
                from scipy.interpolate import griddata
                # Create a grid for interpolation
                grid_x, grid_z = np.mgrid[min(q_x_filt):max(q_x_filt):100j, min(q_z_filt):max(q_z_filt):100j]

                # Interpolate the data
                grid_intensity = griddata((q_x_filt, q_z_filt), plot_intensity, (grid_x, grid_z), method='linear')
                
                # Set up the 3D plot
                surf = ax.plot_surface(grid_x, grid_z, grid_intensity, cmap=cm.coolwarm, linewidth=0, antialiased=False)
                
                # Add a color bar
                # Add a color bar which maps values to colors.
                cbar = plt.colorbar(surf, shrink=0.5, aspect=5, pad=0.02, format=ticker.FuncFormatter(lambda x, _: f"{x:.1f}"))
